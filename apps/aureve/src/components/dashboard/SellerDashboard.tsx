...
export function SellerDashboard({ profile }: { profile: any }) {
  ...
  return (
    <div>
      ...
      <div style={{ background: 'linear-gradient(135deg, #1A1A16 0%, #2C2C28 100%)', borderRadius: '16px', padding: '1.75rem 2rem', display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem', border: '1px solid rgba(29,158,117,0.2)', position: 'relative', overflow: 'hidden' }}>
        ...
        <div>
          ...
          <h3 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.5rem', color: '#FAF8F3', marginBottom: '4px' }}>List your property on Aureve</h3>
          <p style={{ fontSize: '12px', color: 'rgba(250,248,243,0.45)', fontFamily: 'DM Sans, sans-serif' }}>Reach thousands of verified buyers across Hyderabad</p>
        </div>
        <Link href="/dashboard/listings/new" style={{ background: '#1D9E75', color: '#fff', padding: '11px 22px', borderRadius: '6px', fontSize: '12px', fontWeight: 600, textDecoration: 'none', whiteSpace: 'nowrap', letterSpacing: '0.06em', textTransform: 'uppercase', fontFamily: 'DM Sans, sans-serif' }}>
          ✦ Add listing
        </Link>
      </div>
      ...
      <div style={{ background: '#fff', borderRadius: '16px', padding: '1.75rem', border: '1px solid rgba(26,26,22,0.08)' }}>
        ...
        {[
          { num: '01', tip: 'Add 5+ high quality photos',  desc: 'Listings with multiple photos get 3x more enquiries from serious buyers' },
          ...
          <div key={i} style={{ display: 'flex', gap: '16px', padding: '14px 0', borderBottom: i < 3 ? '1px solid rgba(26,26,22,0.05)' : 'none', alignItems: 'flex-start' }}>
            <div style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.5rem', fontWeight: 300, color: 'rgba(201,168,76,0.3)', lineHeight: 1, flexShrink: 0, width: '32px' }}>{t.num}</div>
            <div>
